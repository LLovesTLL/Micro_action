...
def build_dataset(is_train, test_mode, args):
    ...
    if args.data_set == 'MyCustomDataset':
        mode = None
        anno_path = None
        if is_train is True:
            mode = 'train'
            anno_path = os.path.join(args.data_path, 'annotations/train_list_videos.txt')
            # 训练集视频的根目录
            prefix = os.path.join(args.data_path, 'train') 
        elif test_mode is True:
            mode = 'test'
            anno_path = os.path.join(args.data_path, 'annotations/val_list_videos.txt') 
            # 测试集使用验证集视频
            prefix = os.path.join(args.data_path, 'val') 
        else:  
            mode = 'validation'
            anno_path = os.path.join(args.data_path, 'annotations/val_list_videos.txt') 
            # 验证集视频的根目录
