...
### 📅 2026年3月6日
**任务**: 训练故障深度诊断与“返璞归真”实验方案 (Experiment 5)

#### A. 故障诊断：为何模型“躺平”在 11.47%？
在 Experiment 4 (Aggressive Strategy) 中，尽管我们大幅提升了学习率并移除了层级衰减，模型依然顽固地将验证集准确率锁定在 **11.47%**。为了查明真相，我们进行了一系列细致的诊断：

1.  **数据完整性排查 (`check_data_stats.py`)**：
    *   **怀疑**: 输入数据是否全是黑屏或损坏？
    *   **结果**: 否定。随机抽样的 Batch 数据 `Mean` 约为 0，`Std` 约为 1，且没有标准差为 0 的样本。数据加载正常。

2.  **代码逻辑排查 (`sanity_check.py`)**：
    *   **怀疑**: 模型代码是否有 Bug 导致梯度无法回传？
    *   **结果**: 否定。使用随机生成的伪数据和标签，模型能在 100 步内将 Loss 降至 0.23，准确率达到 100%。证明模型具备正常的学习和过拟合能力。

3.  **统计学“实锤” (`analyze_labels.py`)**：
    *   **发现**: 统计全量数据集的标签分布，发现 **第 9 类 (Class 9)** 的样本数量为 1283/11250 (train) 和 641/5586 (val)。
    *   **计算**: $641 \div 5586 = 0.114751...$
    *   **结论**: **11.47% 正是验证集中占比最大的类别的频率**。
    *   **定性**: 模型发生 **Posterior Collapse (后验坍塌)**。它发现与其费力去从这种嘈杂的微动作视频中提取特征，不如直接忽略输入，永远猜测“第 9 类”，这样能获得一个不算太高的全局 Loss。

#### B. 根本原因分析 (Root Cause Analysis)
为何模型会陷入这种“局部最优”的陷阱？
1.  **预训练特征失效**: 我们使用的是 **ImageNet-1K** 预训练权重。虽然它懂“狗”和“猫”的样子，但完全不懂“点头”和“眨眼”这种极短时间内的微小幅度运动。
2.  **增强策略过激 (AutoAugment + Mixup)**:
    *   默认配置使用了极强的 **Mixup (0.8)** 和 **Cutmix (1.0)**。
    *   对于微动作（Micro-action），动作幅度本来就只有几个像素。Mixup 把两段视频及其标签按比例混合，导致微弱的运动信号被彻底淹没在噪声中。
    *   模型在通过这种“混合视频”去预测“混合标签”时感到困惑，最终选择了一条阻力最小的路径——猜先验概率。
3.  **学习率策略冲突**: Exp 4 激进的 `1e-3` 学习率配合无 `Layer Decay`，可能在训练初期就破坏了 ImageNet 仅存的一点空间特征（如边缘检测），导致模型彻底变成随机初始化。

#### C. 解决方案：Experiment 5 (No Augmentation)
针对上述分析，制定了“**返璞归真 (Back to Basics)**”的实验方案 `exp/Fifth/run_train_noaug.sh`：

1.  **移除干扰**:
    *   `--mixup 0.0`, `--cutmix 0.0`: 禁用所有混合增强，让模型看到干净、原始的视频帧。
    *   `--color_jitter 0.0`: 禁用颜色抖动，保留微表情中的细微光影变化。
2.  **温和的优化**:
    *   **LR**: `5e-4` (比激进的 1e-3 减半，保护特征)。
    *   **Layer Decay**: `0.8` (恢复分层衰减，保护底层的 ImageNet 纹理特征)。
3.  **时序增强**:
